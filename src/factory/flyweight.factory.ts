export class FlyweightFactory<T> {
  private _flyweights: Map<string, IFlyweight<T>> = new Map<
    string,
    IFlyweight<T>
  >()
  ...
  private getKey(state: T) {
    return Object.keys(state).reduce((key: string, property: string) => {
      ...
      if (typeof value === 'object') {
        throw new Error(
          "Please don't nest objects in the state object passed to the getFlyweight method. It only supports shallow object without other objects nested."
        )
      }

      return `${key}_${property}_${value.toString()}_`
    }, '')
  }
}
